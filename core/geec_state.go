...
func (gs *GeecState) Init(chain interface{}, db interface{}, coinbase common.Address) error { //TODO: set parameters
	...
	myIP, myPort := gs.bc.engine.GetConsensusIPPort()

	gs.es = election.NewServer(myIP, myPort, coinbase, gs)
...
func (gs *GeecState) Validate (req *geecCore.ValidateRequest){
	...
	if isValidator{
		log.Gdbug("I am validator, sending reply", "Blk", req.BlockNum)
	}else{
		log.Gdbug("I am not validator", "Blk", req.BlockNum)
		return
	}
...
func (gs *GeecState) ElectForProposer(blkNum uint64, version uint64, stop <-chan struct{}) int{
		...
		rand, exist := gs.GetTrustRand(blkNum-1)
		if !exist {
		...
		ep.Candidates = gs.getAllCommittee(rand)
...
func (gs *GeecState) IsCommittee(blknum uint64) (result bool){
	//First check whether I am candidate
	//check against my coin addreess.
...
func (gs *GeecState) handleTimeout(){
