...
func (g *Geec) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	isCommittee := g.gs.IsCommittee(header.Number.Uint64())
...
func (g *Geec) Seal (chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error){
	...
	if g.failureTest == true && blkNum == 100{
	...
	if g.txnPerBlock - nGeecTxn > 0{
		fakeData := make([]byte, (g.txnDataLen + 31) * (g.txnPerBlock - nGeecTxn))
	...
	log.Gdbug("Checking emptyList", "number", blkNum, "list", g.gs.EmptyBlockList)
...
func (g *Geec) CalcConfidence(chain consensus.ChainReader, block *types.Block) uint64{
	...
	var c uint64
	c = parent.ConfirmMessage.Confidence + 1000
	if c > 10000 {
		return  10000
	}else{
		return c
