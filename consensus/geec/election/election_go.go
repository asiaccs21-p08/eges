...
func (s *Server) Elect(ep *ElectParameters, stop <-chan struct{}) int{
	...
	for {
		...
		wb.Mu.Lock()
		if retry > 0 {
			log.Geec("retry electing for block", "blknum", ep.BlkNum, "retry", retry, "state", wb.ElectState, "supporters", wb.Supporters.Size())
		} else{
		...
		case blk := <-s.electSuccessCh:
			...
			if blk == ep.BlkNum {
				...
				wb.Mu.Unlock()
				log.Gdbug("received success not for me", "received", blk, "waiting", ep.BlkNum )
				continue
			}
...
func (s *Server) handleElectMessage() {
	...
	for {
		...
		log.Gdbug("received elect message", "author", em.Author, "code", em.Code, "blk", em.BlockNum)
			...
			case MSG_ELECT:
				if wb.ElectState == geecCore.ELEC_Candidate{
					if wb.MyRand > em.Rand || (wb.MyRand == em.Rand && AddrToInt(s.coinbase) > AddrToInt(em.Author)) {
						...
						s.vote(wb, em.BlockNum, em.Ipstring, em.Port)
					}
					...
					*/
					...
					if em.Author == wb.Delegator || em.Retry > wb.Max_Election_Retry + 1 {
						...
						s.vote(wb, em.BlockNum, wb.Delegator_Ipstr, wb.Delegator_Port)
...
func (s *Server) vote (wb *geecCore.WorkingBlock, BlockNum uint64, Ipstring string, Port uint){
	reply := new(electMessage)
